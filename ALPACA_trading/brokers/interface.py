...
BROKER INTEGRATION COMPLIANCE NOTICE:
-------------------------------------
This module provides client-side trading execution software that connects to
user-owned brokerage accounts via user-provided API keys.
...
- Users are solely responsible for: brokerage relationships, regulatory compliance,
  API credentials, and all trading decisions
- This is NON-CUSTODIAL execution on USER-OWNED accounts
- Users must comply with their broker's terms of service and all applicable regulations

See LEGAL/BROKER_INTEGRATION_COMPLIANCE.md for complete compliance framework.
"""
...
class Broker(Protocol):
    ...
    This protocol defines the interface for broker adapters that execute trades
    on behalf of user-owned accounts via user-provided API credentials.
    
    COMPLIANCE: All broker implementations must:
    - Use user-provided API credentials (never store or manage credentials)
    - Execute trades only on user-authorized accounts
    - Maintain complete audit trails
    - Support user oversight and intervention
    
    See LEGAL/BROKER_INTEGRATION_COMPLIANCE.md for compliance requirements.
    """
