...
- **Interval detection warning noise reduction** (2025-12-11) — **IMPROVED**: Downgraded "unclear interval" warnings from WARNING to INFO level. These messages are expected when large gaps are filtered out and the default is being used correctly. Added `interval_detection.mode=fixed` config option to skip auto-detection entirely when interval is known. When `mode=fixed`, uses `data.bar_interval` directly without auto-detection, eliminating noise for known intervals. All interval detection messages now at INFO/DEBUG level (not WARNING).
