...
def train_changepoint_heuristic(X, y, config):
    ...
    try:
        ...
        X_train, X_val, y_train, y_val = train_test_split(X_with_changes, y_clean, test_size=test_size, seed=seed)
...
def train_ftrl_proximal(X, y, config):
    ...
    try:
        ...
        X_train, X_val, y_train, y_val = train_test_split(X_scaled, y_clean, test_size=test_size, seed=seed)
...
def train_ensemble(X, y, config):
    ...
    try:
        ...
        # Load hyperparameters from config
        dt_seed = 42  # Default fallback
        rf_n_estimators = 50  # Default fallback
        rf_seed = 42  # Default fallback
        if _CONFIG_AVAILABLE:
            try:
        ...
        models = [
            ('lr', LinearRegression()),
            ('dt', DecisionTreeRegressor(seed=dt_seed)),
            ('rf', RandomForestRegressor(n_estimators=rf_n_estimators, seed=rf_seed))
        ]
        ...
        X_train, X_val, y_train, y_val = train_test_split(X_clean, y_clean, test_size=test_size, seed=seed)
...
def train_meta_learning(X, y, config):
    ...
    try:
        ...
        try:
            ...
            pass
        kf = KFold(n_splits=5, shuffle=True, seed=seed)
