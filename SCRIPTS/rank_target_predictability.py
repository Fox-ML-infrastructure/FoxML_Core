...
# Import reproducibility tracker
...
# Import determinism system for BASE_SEED
from TRAINING.common.determinism import BASE_SEED

# Import config loader for SST-compliant defaults
try:
...
def load_sample_data(
    symbol: str,
    data_dir: Path,
    max_samples: int = None  # Load from config if None
) -> pd.DataFrame:
    """Load sample data for a symbol"""
    # Load max_samples from config if not provided (SST)
    if max_samples is None:
        if _CONFIG_AVAILABLE:
            ...
            max_samples = 10000  # FALLBACK_DEFAULT_OK
    ...
    # Sample if too large (use BASE_SEED for SST)
    if len(df) > max_samples:
        df = df.sample(n=max_samples, random_state=BASE_SEED)
    ...
    if original_feature_count > 100:  # Only prune if we have many features
        ...
        try:
            X_pruned, feature_names_pruned, pruning_stats = quick_importance_prune(
                ...
                random_state=BASE_SEED  # SST: use centralized determinism
            )
...
def evaluate_target_predictability(
    ...
    # Compare to previous run for reproducibility verification
...
def main():
    ...
    # Load config defaults for SST compliance
    if _CONFIG_AVAILABLE:
        default_min_cs = int(get_cfg("pipeline.data_limits.min_cs", default=10, config_name="pipeline_config"))
        ...
        default_min_cs = 10  # FALLBACK_DEFAULT_OK
        default_max_cs_samples = 1000  # FALLBACK_DEFAULT_OK
        default_max_rows = 50000  # FALLBACK_DEFAULT_OK
    
    parser.add_argument("--symbols", type=str, default=None,
                       help="Symbols to test on (comma-separated, default: load from config or use AAPL,MSFT,GOOGL,TSLA,JPM)")
    ...
    parser.add_argument("--min-cs", type=int, default=default_min_cs,
                       help=f"Minimum cross-sectional size per timestamp (default: {default_min_cs} from config)")
    parser.add_argument("--max-cs-samples", type=int, default=default_max_cs_samples,
                       help=f"Maximum samples per timestamp for cross-sectional sampling (default: {default_max_cs_samples} from config)")
    parser.add_argument("--max-rows-per-symbol", type=int, default=default_max_rows,
                       help=f"Maximum rows to load per symbol (most recent rows, default: {default_max_rows} from config)")
    
    args = parser.parse_args()
    
    # Parse inputs (handle None default for symbols)
    if args.symbols is None:
        # Fallback to default symbols if not provided and not in config
        args.symbols = "AAPL,MSFT,GOOGL,TSLA,JPM"  # FALLBACK_DEFAULT_OK
